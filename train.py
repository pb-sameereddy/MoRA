...
class ResetReloraCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if state.global_step % self.T == 0 and state.global_step > 0:
            ...
            print(
                "save base model",
                os.path.join(SAVE_PATH, "base-model"),
            ...
            if self.reset_optimizer:
                ...
                if not self.relora_scheduler:
                    ...
                    print(
                        "reset scheduler", scheduler._last_lr[0], scheduler.state_dict()
                    )
            else:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.lora_plus_lambda > 1:
                lora_b_params = set(
                    [n for n, p in opt_model.named_parameters() if "lora_B" in n]
                )
        ...
        if self.lr_scheduler is None:
            if self.relora_scheduler:
                ...
                if num_training_steps % self.use_relora_step > 0:
                    num_training_steps = (
                        (num_training_steps // self.use_relora_step) + 1
                    ) * self.use_relora_step
...
def train(
    ...
    if debug:
        ...
        load_in_8bit = (
            False
            if full_ft or (deepspeed and "ds3" in deepspeed) or use_16bit
            else True
        )
    ...
    trainer = TRAINER_CLS(
        ...
        args=transformers.TrainingArguments(
            ...
            fsdp=(
                ...
                if full_ft and not deepspeed and pretrain is None
                else ""
            ),
